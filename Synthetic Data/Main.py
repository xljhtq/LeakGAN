...
PRE_EPOCH_NUM = 2  # generator for MLE
generated_num = 500
...
def pre_train_epoch(sess, trainable_model, data_loader):
    ...
    for it in range(data_loader.num_batch):
        batch = data_loader.next_batch()
        _, g_loss, _, _, Worker_Reward = trainable_model.pretrain_step(sess, batch, 1.0)
...
def get_reward(model, dis, sess, input_x, rollout_num, dis_dropout_keep_prob):
    ...
    for i in range(rollout_num):
        for given_num in range(1, model.sequence_length / model.step_size):
            ...
            feed = {model.x: input_x,
                    model.given_num: real_given_num,
                    model.drop_out: 1.0}
            samples = sess.run(model.gen_for_reward, feed)  ## 根据real_given_num来MC生成

            feed = {dis.D_input_x: samples,
    ...
    rewards = np.transpose(np.array(rewards)) / (1.0 * rollout_num)  # batch_size x seq_length/step_size
    return rewards
