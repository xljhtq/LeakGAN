...
class LeakGAN(object):
    def __init__(self,
        ...
        learning_rate_pre = tf.train.exponential_decay(self.starter_learning_rate,
                                                       global_step_pre,
        ...
        with tf.variable_scope('Worker'):  ## todo: modify g_embeddings
            ...
            self.g_change = tf.Variable(tf.random_normal([self.goal_out_size, self.goal_size], stddev=0.1))  ## 600*16
        ...
        feature_array = tensor_array_ops.TensorArray(dtype=tf.float32,
        ...
        gen_real_goal_array = tensor_array_ops.TensorArray(dtype=tf.float32,
                                                           size=self.sequence_length,
                                                           dynamic_size=False,
                                                           infer_shape=True,
                                                           clear_after_read=False)
        ...
        def _g_recurrence(i, x_t, h_tm1, h_tm1_manager, gen_o, gen_x, goal, last_goal, real_goal, step_size,
            ...
            with tf.variable_scope(self.scope):
                feature = self.FeatureExtractor_unit(cur_sen, self.drop_out)  ## 64 * 40
            h_t_Worker = self.g_worker_recurrent_unit(x_t, h_tm1)
            o_t_Worker = self.g_worker_output_unit(h_t_Worker)  # batch * (vocab * goal_size)
            ...
            real_sub_goal = tf.add(last_goal, sub_goal)  ## g

            w_g = tf.matmul(real_goal, self.g_change)  # batch * goal_size, real_goal就是上一次的real_sub_goal
            w_g = tf.nn.l2_normalize(w_g, 1)
            w_g = tf.expand_dims(w_g, 2)  # batch * goal_size * 1
            gen_real_goal_array = gen_real_goal_array.write(i, real_goal)

            x_logits = tf.matmul(o_t_Worker, w_g)  ## todo: 64 * 5000 * 1
            x_logits = tf.squeeze(x_logits)  ## 64 * 5000

            log_prob = tf.log(tf.nn.softmax(
                tf.cond(i > 1,
                        lambda: tf.cond(self.train > 0,
                                        lambda: self.tem,
                                        lambda: 1.5),
                        lambda: 1.5) *
                x_logits))
        ...
        self.gen_o_worker_array = self.gen_o_worker_array.stack()  # seq_length * batch_size * vocab * goal

        self.gen_o_worker_array = tf.transpose(self.gen_o_worker_array,
                                               perm=[1, 0, 2, 3])  # batch_size * seq_length * vocab * goal
        ...
        all_sub_features = tensor_array_ops.TensorArray(dtype=tf.float32,
                                                        size=self.sequence_length,
                                                        dynamic_size=False,
                                                        infer_shape=True,
                                                        clear_after_read=False)
        ...
        print ("supervised pretraining for generator")
        ...
        def preTrain(i,
                     ...
                     all_sub_goals):
            ## padding sentence by -1
            cur_sen = tf.split(tf.concat([tf.split(input_x, [i, self.sequence_length - i], 1)[0],
                                          self.padding_array], 1),
                               [self.sequence_length, i], 1)[0]  # 根据i的大小来padding sentence
            with tf.variable_scope(self.scope):
                feature = self.FeatureExtractor_unit(cur_sen, self.drop_out)
            feature_array = feature_array.write(i, feature)
            real_goal_array = tf.cond(i > 0,
                                      lambda: real_goal_array,
                                      lambda: real_goal_array.write(0, self.goal_init))
            h_t_manager = self.g_manager_recurrent_unit(feature, h_tm1_manager)
            sub_goal = self.g_manager_output_unit(h_t_manager)
            sub_goal = tf.nn.l2_normalize(sub_goal, 1)  ## 64 * 600

            h_t_Worker = tf.cond(i > 0,
                                 lambda: self.g_worker_recurrent_unit(x_t, h_tm1),
                                 lambda: h_tm1)
            o_t_Worker = self.g_worker_output_unit(h_t_Worker)  # batch * (vocab * goal)
            o_t_Worker = tf.reshape(o_t_Worker, [self.batch_size, self.vocab_size, self.goal_size])

            real_sub_goal = tf.cond(i > 0,
                                    lambda: tf.add(last_goal, sub_goal),
                                    lambda: real_goal)
            all_sub_goals = tf.cond(i > 0,
                                    lambda: all_sub_goals.write(i - 1, real_goal),  ## 40 * 64 * 600
                                    lambda: all_sub_goals)

            w_g = tf.matmul(real_goal, self.g_change)  # todo: real_goal 每过step_size才会变 batch x goal_size
            w_g = tf.nn.l2_normalize(w_g, 1)
            ...
            g_predictions = tf.cond(i > 0,
                                    lambda: g_predictions.write(i - 1, tf.nn.softmax(x_logits)),
                                    lambda: g_predictions)

            sub_feature = tf.cond(((((i) % step_size) > 0)),  ##64*10*600 todo: i=step_size, 2*step_size  ...时候-1 是为了错开一个step_size
                                  lambda: sub_feature,
                                  lambda: (tf.cond(i > 0,
                                                   lambda: sub_feature.write(i / step_size - 1,
                                                                             tf.subtract(feature,
                                                                                         feature_array.read(
                                                                                             i - step_size))),
                                                   lambda: sub_feature)))

            all_sub_features = tf.cond(i > 0,     ##
                                       lambda: tf.cond((i % step_size) > 0,
                                                       lambda: all_sub_features.write(i - 1,
                                                                                      tf.subtract(
                                                                                          feature,
                                                                                          feature_array.read(
                                                                                              i - i % step_size))),
            ...
            real_goal_array = tf.cond(((i) % step_size) > 0,  ##64*10*600 todo:  i=step_size, 2*step_size  ...
                                      lambda: real_goal_array,
        ...
        self.sub_feature = self.sub_feature.stack()  # (sequence_length / step_size) x batch_size x 600
        self.sub_feature = tf.transpose(self.sub_feature, perm=[1, 0, 2])

        self.real_goal_array = self.real_goal_array.stack()  # (sequence_length / step_size) x batch_size x 600
        self.real_goal_array = tf.transpose(self.real_goal_array, perm=[1, 0, 2])
        print("real_goal_array: ", self.real_goal_array.shape)
        print("sub_feature: ", self.sub_feature.shape)
        ...
        with tf.name_scope("Manager_PreTrain_update"):
            ...
            self.pretrain_manager_grad, _ = tf.clip_by_global_norm(tf.gradients(self.pretrain_goal_loss,
                                                                                self.manager_params),
                                                                   self.grad_clip)
            self.pretrain_manager_updates = pretrain_manager_opt.apply_gradients(list(zip(self.pretrain_manager_grad,
                                                                                          self.manager_params)),
        ...
        ) / (self.sequence_length * self.batch_size)  ## MLE
        ...
        with tf.name_scope("Worker_PreTrain_update"):
            ...
            self.pretrain_worker_updates = pretrain_worker_opt.apply_gradients(
                list(zip(self.pretrain_worker_grad, self.worker_params)), global_step=global_step_pre)

        self.goal_loss = -tf.reduce_sum(
            tf.multiply(self.reward, 1 - tf.losses.cosine_distance(tf.nn.l2_normalize(self.sub_feature, 2),
                                                                   tf.nn.l2_normalize(self.real_goal_array, 2),
                                                                   2)
                        )) / (self.sequence_length * self.batch_size / self.step_size)
        ...
        self.all_sub_features = tf.transpose(self.all_sub_features, perm=[1, 0, 2])  ## 64*40*600

        self.all_sub_goals = self.all_sub_goals.stack()
        self.all_sub_goals = tf.transpose(self.all_sub_goals, perm=[1, 0, 2]) ## 64*40*600
        self.Worker_Reward = 1 - tf.losses.cosine_distance(tf.nn.l2_normalize(self.all_sub_features, 2),
                                                           tf.nn.l2_normalize(self.all_sub_goals, 2),
                                                           2)  ## TODO: ??
        print ("Worker_Reward.shape: ", self.Worker_Reward.shape)
    ...
    def rollout(self, input_x, given_num):
        ...
        ta_x = ta_x.unstack(tf.transpose(input_x, perm=[1, 0]))  ## 40 * 64 的unstack
